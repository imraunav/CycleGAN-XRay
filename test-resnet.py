...
in_path = "./test/input"
fused_path = "./test/fused-resnet"
back_path = "./test/back-resnet"
weights12 = "./weights/g12resnet_1000_loss0.0557.pt"
weights21 = "./weights/g21resnet_1000_loss0.0557.pt"


def main():
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

    resnet21 = ResNet(2, 1).eval()
    resnet21.load_state_dict(torch.load(weights21, map_location=device))
    print("resnet21 weights loaded successfully!")

    resnet12 = ResNet(1, 2).eval()
    resnet12.load_state_dict(torch.load(weights12, map_location=device))
    print("resnet12 weights loaded successfully!")
    ...
    for low, high in data:
        ...
        fusedtensor = resnet21(intensor).sigmoid()

        backtensor = resnet12(fusedtensor).sigmoid()
